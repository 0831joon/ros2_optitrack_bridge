...
void OptitrackBridgePX4Node::loop_() {
    ...
    // mocap_odom_msg.timestamp = this->now().nanoseconds() / 1000;
    // mocap_odom_msg.timestamp_sample = (static_cast<uint64_t>(pose_msg.header.stamp.sec) * 1000000ULL)+(pose_msg.header.stamp.nanosec / 1000);


    visual_odom_msg.pose_frame = 1;
    // mocap_odom_msg.pose_frame = 1;
    ...
    // mocap_odom_msg.position[0] = pose_msg.pose.position.x;
    // mocap_odom_msg.position[1] = -pose_msg.pose.position.y;
    // mocap_odom_msg.position[2] = -pose_msg.pose.position.z;
    ...
    // mocap_odom_msg.q[3] = pose_msg.pose.orientation.w;
    ...
    // for (int i = 0; i < 3; ++i) {
    //     mocap_odom_msg.velocity[i] = nan_val;
    //     mocap_odom_msg.angular_velocity[i] = nan_val;
    //     mocap_odom_msg.position_variance[i] = nan_val;
    //     mocap_odom_msg.orientation_variance[i] = nan_val;
    //     mocap_odom_msg.velocity_variance[i] = nan_val;
    // }

    publisher_vehicle_visual_odometry_->publish(visual_odom_msg);
    // publisher_vehicle_mocap_odometry_->publish(mocap_odom_msg);
